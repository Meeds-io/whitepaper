...
ה-WoM יספק בסופו של דבר כמות מוגבלת של Deeds. Each Deed includes a predefined maximum number of users and offers a capacity of rewarding (Minting Power). המונח 'משתמש' כאן ולהלן מציין כל סוג של עובד, עובד, פרילנסר, מתנדב וכו'.
...
בתוך רכזות אלה, מנהיגי קהילה יכולים לעודד מעורבות של חברים על ידי הצעת תפריט שקוף של תמריצים לפריטי עבודה. Participants earn engagement points for their efforts, which are periodically converted into MEED tokens or a custom equivalent, in line with a predetermined budget.
...
חלוקות קבועות של תגמולים מבוססות על הנקודות שנצברו בכל רכזת. בדרך כלל, אלה ניתנים באסימוני MEED, המועברים ישירות לארנקים של המשתמשים. בעוד שאסימוני MEED הם הצורה הסטנדרטית של תגמול, לרכזות יש את הגמישות להציע סוגים שונים של פיצויים.

כאשר תגמולים מחולקים באסימונים שאינם MEED, יחולו עמלה סמלית של 5%. עמלה זו תורמת לאוצר מידס, וממלאת תפקיד מכריע בשמירה על קיימות המערכת האקולוגית.
...
Each week, a certain amount of MEED is allocated to the global engagement minting of the WoM. הוא מחושב מדי שבוע, באופן יחסי, בין המרכזים המשתתפים. כמה רכזות לקבל דחיפה נוספת בהתאם לסוג המעשה הם משתמשים. The Hubs that claim their MEED tokens over the week, receive them. The unclaimed MEED tokens are added to the following week’s global distribution amount.
...
- $D_r$ = _שיעור הפצה_ - אם Hub קיבל MEED מוטבע עבור התקופה הקודמת, אחוז ה-MEED שהופץ למשתמשים שלו. ה-% יכול להיות מעל 100 אם ההפצה מתעכבת בזמן.
- $D_s$ = _פיזור הפצה_ - אם Hub קיבל אסימוני MEED מוטבעים עבור התקופה הקודמת, אחוז המשתמשים שקיבלו MEED במאגר המשתמשים הכולל.
...
> The Meeds Association reserves the right to change the formula if required – if some specific use cases arise, for instance. העמותה יכולה גם לרשום כל רכזת שחוטפת את הנוסחה לטובתה הייחודית.

As the Hubs are rewarded with MEED and incentivized to redistribute them as rewards to their users against their engagement – the interests of both the communities and their users are aligned in the same virtuous revenue-sharing cycle. לפיכך, ה-WoM מציע מודל עבודה חדש, המעצב מחדש את יחסי המעסיק/עובד כשותפות.
...
לדוגמה, הם יכולים להוסיף ייצוג דיגיטלי של העיר שלהם, עמיתים פיזיים הקשורים לעיר שלהם (לדוגמה חדרי ישיבות בחללי עבודה משותפים פיזיים), להוסיף שירותי מזון, שירותי נופש וכו'...
